...
import { newHttpBatchRpcSession as newHttpBatchRpcSessionImpl,
         ...
         type HttpBatchRpcSessionOptions } from "./batch.js";
...
export type { RpcTransport, RpcSessionOptions, RpcCompatible, HttpBatchRpcSessionOptions };
 ...
 *
 * The URL is the endpoint to send batch requests to. Options can include:
 * - `headers`: Custom headers to include with every batch request (e.g., Authorization)
 * - `fetch`: Custom fetch function (useful for testing or custom HTTP clients)
 * - `onSendError`: Error handling callback inherited from RpcSessionOptions
 *
 * @example
 * ```typescript
 * // With authorization header
 * const session = newHttpBatchRpcSession<MyApi>('https://api.example.com/rpc', {
...
export let newHttpBatchRpcSession:<T extends RpcCompatible<T>>
    (urlOrRequest: string | Request, options?: HttpBatchRpcSessionOptions) => RpcStub<T> =
