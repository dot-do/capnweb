...
/**
 * Options for HTTP batch RPC sessions.
 */
export type HttpBatchRpcSessionOptions = RpcSessionOptions & {
  /**
   * Custom headers to include with every batch request.
   * These headers are merged with the default Content-Type header.
   */
...
export function newHttpBatchRpcSession(
    urlOrRequest: string | Request, options?: HttpBatchRpcSessionOptions): RpcStub {
  const fetchFn = options?.fetch ?? fetch;
  const customHeaders = options?.headers ?? {};

  let sendBatch: SendBatchFunc = async (batch: string[]) => {
    let response = await fetchFn(urlOrRequest, {
      method: "POST",
      body: batch.join("\n"),
      headers: {
        ...customHeaders,
