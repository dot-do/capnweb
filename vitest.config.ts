  ...
  test: {
    ...
    projects: [
      // Node.js
      {
        test: {
          name: 'node',
          include: ['__tests__/index.test.ts', '__tests__/batch-auth.test.ts', '__tests__/websocket-auth.test.ts', '__tests__/do-user-context.test.ts', 'packages/client/tests/**/*.test.ts'],
          environment: 'node',
        },
      },
      ...
      // React (jsdom)
      {
        ...
        resolve: {
          alias: {
            '@dotdo/client': new URL('./packages/client/src/index.ts', import.meta.url).pathname,
          },
        },
        test: {
          name: 'react',
          include: ['packages/react/tests/**/*.test.ts'],
          environment: 'jsdom',
        },
      },
